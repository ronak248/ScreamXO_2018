...
public class MobilePlayerViewController: MPMoviePlayerViewController {
    ...
    public override var title: String? {
        didSet {
            guard let titleLabel = getViewForElementWithIdentifier(identifier: "title") as? Label else {
                return
            }
    ...
    public init(
        ...
        super.init(contentURL: contentURL as URL!)
    ...
    private func initializeMobilePlayerViewController() {
        ...
        if let watermarkConfig = config.watermarkConfig {
            showOverlayViewController(overlayViewController: WatermarkViewController(config: watermarkConfig))
        }
    }
...
//            object: moviePlayer)
//        notificationCenter.addObserver(forName: NSNotification.Name.MPMoviePlayerPlaybackDidFinish,
//            object: moviePlayer,
//            queue: OperationQueue.main) { [weak self] notification in
//                guard let slf = self else {
//                    
...
//                    return
//                }
//                if let userInfo = notification.userInfo as? [String: AnyObject], let error = userInfo["error"] as? NSError {
//                    
//                    NotificationCenter.default.post(name: NSNotification.Name(rawValue: MobilePlayerDidEncounterErrorNotification), object: slf, userInfo: [MobilePlayerErrorUserInfoKey: error])
//                }
//                if let postrollVC = slf.postrollViewController {
//                    slf.prerollViewController?.dismiss()
//                    slf.pauseOverlayViewController?.dismiss()
//                    slf.showOverlayViewController(overlayViewController: postrollVC)
//                }
//                
//                self!.play()
//        }
    ...
    private func initializeControlsView() {
        (getViewForElementWithIdentifier(identifier: "playback") as? Slider)?.delegate = self
        
        _ = (getViewForElementWithIdentifier(identifier: "close") as? Button)?.addCallback(
            callback: { [weak self] in
                ...
                NotificationCenter.default.post(name: NSNotification.Name(rawValue: "forVideoPlayinglanscape"), object: nil)
                
            },
            forControlEvents: .touchUpInside)
        
        if let actionButton = getViewForElementWithIdentifier(identifier: "action") as? Button {
            actionButton.isHidden = true // Initially hidden until 1 or more `activityItems` are set.
            _ = actionButton.addCallback(
                callback: { [weak self] in
                    guard let slf = self else {
                        return
                    }
        ...
        _ = (getViewForElementWithIdentifier(identifier: "play") as? ToggleButton)?.addCallback(
            callback: { [weak self] in
                guard let slf = self else {
                    return
                }
    ...
    public override func viewDidLoad() {
        ...
        if let prerollViewController = prerollViewController {
            shouldAutoplay = true
            showOverlayViewController(overlayViewController: prerollViewController)
        }
...
//        setNeedsStatusBarAppearanceUpdate()
    ...
    public func play() {
        moviePlayer.play()
//        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "PlayNext"), object: nil)
    }
    ...
    public var activityItems: [URL]? {
        didSet {
            ...
            getViewForElementWithIdentifier(identifier: "action")?.isHidden = (isEmpty == nil || isEmpty == true)
        }
    }
    ...
    public func showContentActions(sourceView: UIView? = nil) {
        ...
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "sharemedia"), object: nil)
    ...
    private func parseContentURLIfNeeded() {
        ...
        YoutubeParser.h264videosWithYoutubeID(youtubeID: youtubeID) { videoInfo, error in
            if let error = error {
            ...
            if let
                ...
                let previewImageURL = NSURL(string: previewImageURLString) {
                URLSession.shared.dataTask(with: previewImageURL as URL) { data, response, error in
            ...
            if let videoURL = videoInfo.videoURL {
                self.moviePlayer.contentURL = NSURL(string: videoURL)! as URL
            }
        }
    }
    ...
    private func updatePlaybackInterface() {
        if let playbackSlider = getViewForElementWithIdentifier(identifier: "playback") as? Slider {
        ...
        if let currentTimeLabel = getViewForElementWithIdentifier(identifier: "currentTime") as? Label {
            currentTimeLabel.text = textForPlaybackTime(time: moviePlayer.currentPlaybackTime)
            currentTimeLabel.superview?.setNeedsLayout()
        }
        if let remainingTimeLabel = getViewForElementWithIdentifier(identifier: "remainingTime") as? Label {
        ...
        if let durationLabel = getViewForElementWithIdentifier(identifier: "duration") as? Label {
    ...
    private func handleMoviePlayerPlaybackStateDidChangeNotification() {
        ...
        let playButton = getViewForElementWithIdentifier(identifier: "play") as? ToggleButton
        ...
        if state == .Playing {
            ...
            if let pauseOverlayViewController = pauseOverlayViewController, (state == .Paused && !seeking) {
                showOverlayViewController(overlayViewController: pauseOverlayViewController)
            }
        }
    }
            ...
            for timedOverlayInfo in self.timedOverlays {
                if timedOverlayInfo.startTime <= currentTime && currentTime <= timedOverlayInfo.startTime + timedOverlayInfo.duration {
                    if timedOverlayInfo.overlay.parent == nil {
                        DispatchQueue.main.async() {
                            self.showOverlayViewController(overlayViewController: timedOverlayInfo.overlay)
                        }
                    }
