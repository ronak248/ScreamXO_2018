...
open class SweetAlert: UIViewController {
    ...
    let kFont = "ProximaNova"
    let kSubFont = "ProximaNova-Regular"
    ...
    init() {
        ...
        self.view.backgroundColor = UIColor.white //UIColor(red:0, green:0, blue:0, alpha:kBakcgroundTansperancy)
    ...
    fileprivate func setupContentView() {
        ...
        contentView.backgroundColor = UIColor.white //colorFromRGB(0x9B9B9B)
        contentView.layer.borderColor = UIColor.colorFromRGB(0x9B9B9B).cgColor
    ...
    fileprivate func setupTitleLabel() {
        ...
        titleLabel.textColor = colors.kLightblack
    }
    ...
    fileprivate func setupSubtitleTextView() {
        ...
        subTitleTextView.font = UIFont(name: kSubFont, size:16)
        subTitleTextView.textColor = UIColor.black
    ...
    fileprivate func resizeAndRelayout() {
        ...
        if self.subTitleTextView.text.isEmpty == false {
            ...
            textViewHeight = ceil(rect.size.height) + 30.0
        ...
        for button in buttons {
           ...
           // buttonRect.append(string.boundingRectWithSize(CGSize(width: width, height:0.0), options: NSStringDrawingOptions.UsesLineFragmentOrigin, attributes:[NSFontAttributeName:button.titleLabel!.font], context:nil))
            ...
            if buttons.count == 1
            {
                    buttonRect.append( CGRect(x: 20, y: contentView.frame.size.height-60, width: contentView.frame.size.width-40, height: 50))
            }
            else if buttons.count == 2
            {
                if buttonRect.count == 0
                {
                    buttonRect.append( CGRect(x: 20, y: contentView.frame.size.height-60, width: contentView.frame.size.width/2-40, height: 50))
                }
                else
                {
                    buttonRect.append( CGRect(x: contentView.frame.size.width/2+10, y: contentView.frame.size.height-60, width: contentView.frame.size.width/2-40, height: 50))
                }
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String, action: (( _ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
        self.showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: colors.klightgreyfont)
        userAction = action
        return self
    }
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, action: (( _ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
            self.showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: buttonColor,otherButtonTitle:
                otherButtonTitle,otherButtonColor: colors.klightgreyfont)
            userAction = action
            return self
    }
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, otherButtonColor: UIColor?,action: (( _ isOtherButton: Bool) -> Void)? = nil) {
            ...
            if buttonTitle.isEmpty == false {
                ...
                button.tag = 0
                button.backgroundColor = UIColor(red: 250/255, green: 105/255, blue: 106/255, alpha: 1.0)
...
class CancelAnimatedView: AnimatableView {
    ...
    fileprivate func setupLayers() {
        ...
        circleLayer.strokeColor = UIColor(red: 42/255, green: 175/255, blue: 142/255, alpha: 1.0).cgColor //colors.klightgreyfont.cgColor;
        ...
        crossPathLayer.strokeColor = UIColor(red: 42/255, green: 175/255, blue: 142/255, alpha: 1.0).cgColor //colors.klightgreyfont.cgColor;
...
class InfoAnimatedView: AnimatableView {
    ...
    func setupLayers() {
        circleLayer.path = outlineCircle
        circleLayer.fillColor = UIColor.clear.cgColor;
        circleLayer.strokeColor = UIColor(red: 42/255, green: 175/255, blue: 142/255, alpha: 1.0).cgColor //colors.kLightblack.cgColor //UIColor.colorFromRGB(0xF8D486).CGColor;
        circleLayer.lineCap = kCALineCapRound
        circleLayer.lineWidth = 4;
        circleLayer.frame = CGRect(x: 0, y: 0, width: self.frame.size.width, height: self.frame.size.height)
        circleLayer.position = CGPoint(x: self.frame.size.width/2.0, y: self.frame.size.height/2.0)
    ...
    override func animate() {
        ...
        colorAnimation.fromValue = UIColor(red: 250/255, green: 105/255, blue: 106/255, alpha: 1.0).cgColor//colors.kLightblack //UIColor.colorFromRGB(0xF7D58B).CGColor
        colorAnimation.toValue = UIColor(red: 250/255, green: 105/255, blue: 106/255, alpha: 1.0).cgColor //colors.kLightblack //UIColor.colorFromRGB(0xF2A665).CGColor
...
class SuccessAnimatedView: AnimatableView {
    ...
    func setupLayers() {
        ...
        outlineLayer.strokeColor = UIColor(red: 42/255, green: 175/255, blue: 142/255, alpha: 1.0).cgColor //colors.kLightblack.cgColor;
        ...
        circleLayer.fillColor = UIColor.clear.cgColor;
        circleLayer.strokeColor = UIColor(red: 42/255, green: 175/255, blue: 142/255, alpha: 1.0).cgColor
        //colors.kLightblack.cgColor;
        circleLayer.lineCap = kCALineCapRound
        circleLayer.lineWidth = 4;
        circleLayer.actions = [
            "strokeStart": NSNull(),
