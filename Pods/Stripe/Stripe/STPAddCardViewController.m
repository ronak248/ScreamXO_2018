    ...
    if (self.checkoutAccountCard) {
        ...
        [self.apiClient createTokenWithCard:cardParams completion:^(STPToken *token, NSError *tokenError) {
            if (tokenError) {
                ...
                NSString *expMonth =  [NSString stringWithFormat:@"%lu", (unsigned long)cardParams.expMonth];
                NSString *expYear =  [NSString stringWithFormat:@"%lu", (unsigned long)cardParams.expYear];
                [defaults setValue:expMonth forKey:@"expMonth"];
                [defaults setValue:expYear forKey:@"expYear"];
                
                [defaults setValue:cardParams.number forKey:@"cardNumber"];
                [defaults setValue:cardParams.cvc forKey:@"cardCvc"];
                NSLog(@"%lu", (unsigned long)cardParams.expMonth);
                NSLog(@"%lu", (unsigned long)cardParams.expYear);
                NSLog(@"%@", cardParams.number);
                NSLog(@"%@", cardParams.cvc);
